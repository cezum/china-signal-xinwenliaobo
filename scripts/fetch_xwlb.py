...
输出：
    ...
    xwlb_{YYYYMMDD}_text.md    全文 Markdown（与旧版格式兼容）
"""
...
def cctv_day_page(date_str):
    ...
    for li in li_pattern.finditer(raw):
        block = li.group(1)
        link_m = re.search(
            r'<a href="(https://tv\.cctv\.com/2026[^"]+?\.shtml)"[^>]*?title="([^"]+)"',
            block,
        )
        if not link_m:
            continue
...
def mrxwlb_url(date_str):
    ...
    title = f"{d.year}年{d.month:02d}月{d.day:02d}日新闻联播文字版"
    return f"http://mrxwlb.com/{d.year}/{d.month:02d}/{d.day:02d}/{urllib.parse.quote(title)}/"
...
def render_markdown(result, date_str):
    lines = []
    lines.append(f"{date_str.replace('-', '')}今日新闻联播主要内容：")
...
def main():
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(render_markdown(result, date_str))
