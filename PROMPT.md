...
跟踪表服务投资线索——最终回答"哪个产业更有发展"。十五五纲要对照是定位工具（正在报道的事项在更大叙事中的战略位置）；框架判定与生命周期事件是分析资产（每次判断留证据）。

## 信号提取标准（3-4条）

同时满足：有政策载体（会议/文件/领导人讲话/部委规划方案）；有新信息（NEW 或 PROGRESS，REPEAT 不选）；能写出投资假设（"XX获政策倾斜 → 某产业链订单/数据加速"）；可验证（1-3个月内）；相对跟踪表有信息增量。超过4条按"层级高、具体性强、验证窗口近"优先；不够格不硬凑。讣告、灾害报道、纯活动报道不算。
...
4. 日报格式：`# 新闻联播政策信号日报 | YYYY-MM-DD` → `## 今日要点`（2-3段叙事）→ `## 读报指南`（八个口径表：层级/首次性/具体性/政策窗口/验证窗口/叙事框架/投资假设/框架判定依据）→ `## 信号详析`（每条：联播原文标注第N条/趋势判断/投资假设/层级·首次性·具体性·验证窗口/政策窗口/叙事框架附判定依据/待验证）→ `## 信号跟踪表`（列名写全"政策窗口"；更新说明首词用主题状态；附今日生命周期事件表、框架判定一致性检查、假设检验到期检查、叙事框架变化提示）→ `## 风险提示`（仅两类：直接风险事件标注条目；应出现未出现的异常缺席）→ 页脚（数据源 CCTV 央视网/备份镜像；跟踪表主题总数）
5. 不写市场预期/投资者行为推测；验证源仅限当日联播文本
  ...
  "signals": [
    {
      ...
      "new_theme": {
        ...
        "verification": {"condition": "联播报道XX（检验假设）", "source": "联播", "date": "2026-xx-xx", "grace_period": "+30天", "status": "跟踪中"},
        "category": "产业政策与科技创新"
      }
    },
    {
...
- `report_markdown` 必须是完整可用的日报，禁止省略章节
