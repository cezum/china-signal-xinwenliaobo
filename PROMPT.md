...
6. **推送摘要不是标题列表**：`report_summary` 只选当天信息量最大的 2-3 条信号，每条必须同时回答三个问题——"发生了什么"、"为什么值得注意"、"下一步盯什么"。表述上可用"信号点 / 政策含义 / 接下来盯"短句，避免"关注后续数据"这类空话；验证打卡只写有实际结果的项（已验证/证伪/新增待验证），没有值得写的就省略。禁止出现投资建议、订单加速、市场预期等表述
...
```json
{
  ...
  "report_summary": "供推送的2-4句话摘要：每条信号包含信号点+政策含义+接下来盯；验证打卡只写有结果的项；不含 HTML 标签和证据底稿",
