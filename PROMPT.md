...
项目对外定位是**政策偏离检测**：跟踪表服务"规划承诺 vs 联播叙事 vs 现实兑现"之间的偏差，最终回答"哪些产业政策在悄悄加码、被搁置、转向或改口"。投资假设作为内部可证伪验证指标保留，不进公开报告顶层；公开层使用"政策传导逻辑"和"接下来盯"表述，不写投资建议。十五五纲要对照是基线工具（正在报道的事项在更大规划中的战略位置）；框架判定与生命周期事件是分析资产（每次判断留证据）。
...
4. 日报采用"双层结构"：顶部是**速览卡片**，底部用 `<details>` 放**证据与方法底稿**。格式固定为：`# 联播风向标 | YYYY-MM-DD` → `> 今日一句：...` → `## 🎯 今日速览`（每条信号一个 `#### N. 标题`；卡片字段为：联播怎么说 / 行业传导 / 接下来盯 / 叙事与首次性标签；无框架迁移时只写叙事标签，有框架迁移时补充 from→to）→ `## 📌 风险与验证打卡` → `## ⚠️ 风险提示`（仅两类：直接风险事件标注条目；应出现未出现的异常缺席）→ 页脚（数据源 CCTV 央视网/备份镜像；跟踪表主题总数）。`> 今日一句` 只写当天最强的一个变化，不超过 40 个汉字，不得并列多条信号，也不得使用"同日另有……"这类汇总式表达。卡片字段中“联播怎么说”不超过 80 个汉字，“行业传导”不超过 80 个汉字，只保留核心动作和量化结果。卡片层禁止出现"投资假设""订单加速""投资线""订单来源"；如需表达政策到产业的变化，用"产业需求""实物工作量""验证锚点"等中性词。每个速览卡片后必须有一个闭合的 `<details>` 结构，其中 `<summary>` 必须固定且仅写 `📂 证据与方法底稿`，不得把联播原文、维度、假设写进 `<summary>`。底稿正文只写证据骨架，不重复完整联播原文：**联播关键句（第N条，不超过120个汉字）**、维度（层级/首次性/具体性/政策窗口/验证窗口）、叙事框架与命中词、**政策传导假设（内部验证指标）**、验证条件/验证源/验证日期/宽限期、十五五纲要映射、当日生命周期变化（如有）。`<details>` 与 `</details>`、`<summary>` 与 `</summary>` 必须成对出现
...
6. **推送摘要不是标题列表**：`report_summary` 只选当天信息量最大的 2-3 条信号，每条必须同时回答三个问题——"发生了什么"、"为什么值得注意"、"下一步盯什么"。每条信号固定为三行：
   - `信号点：发生了什么`
   - `政策含义：为什么值得注意`
   - `接下来盯：下一步看什么`
   信号之间用空行分隔，不得写成连续一段。**`report_summary` 不包含验证打卡**（日报完整版保留验证打卡），也不包含 HTML 标签；禁止出现投资建议、订单加速、市场预期等表述
...
```json
{
  ...
  "report_summary": "供推送的2-4条信号摘要：每条按 信号点/政策含义/接下来盯 三行输出，信号之间空行；不包含验证打卡、HTML 标签和证据底稿",
